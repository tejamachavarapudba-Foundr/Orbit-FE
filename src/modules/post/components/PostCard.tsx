...
import { PostMenuAction, usePostActionsSheetStore } from "@/modules/post/postActionsSheetStore";
...
export const PostCard = memo(({ post }: PostCardProps) => {
  ...
  const openMenu = useCallback(() => {
    const actions: PostMenuAction[] = isOwnPost
      ? [
          { label: "Copy link", icon: "link", onPress: () => void copyLink() },
          { label: "Edit post", icon: "edit-2", onPress: () => setIsEditing(true) },
          { label: "Delete post", icon: "trash-2", onPress: confirmDelete, destructive: true }
        ]
      : [
          { label: "Copy link", icon: "link", onPress: () => void copyLink() },
          { label: "Not interested", icon: "eye-off", onPress: markNotInterested },
          { label: "Report post", icon: "flag", onPress: reportPost, destructive: true }
        ];
    usePostActionsSheetStore.getState().open(actions);
  }, [confirmDelete, copyLink, isOwnPost, markNotInterested, reportPost]);
